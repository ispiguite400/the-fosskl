...
(function () {
  ...
  /*
   * A pleat: a ridge that runs ALONG a limb (fold() wraps AROUND it), at
   * angle th round the axis a -> b, from t0 to t1. Hakama pleats, the drape
   * of a robe, the long creases down a sleeve or a cape.
   */
  C.pleat = function (a, b, th, o) {
    o = o || {};
    const ax = norm(sub(b, a)), front = o.front || [0, 0, 1];
    const e1 = norm(sub(front, mul(ax, dot(front, ax)))), e2 = cross(ax, e1);
    const dir = addv(mul(e1, Math.cos(th)), mul(e2, Math.sin(th)));
    const t0 = o.t0 === undefined ? 0.05 : o.t0, t1 = o.t1 === undefined ? 0.95 : o.t1, n = o.n || 10;
    const r = o.r || 0.012, h = o.h || 0.005, k = o.k === undefined ? 0.01 : o.k;
    const pts = [];
    for (let i = 0; i < n; i++) {
      const t = t0 + (t1 - t0) * i / (n - 1);
      const s2 = C.surfOut(C.v.lerp(a, b, t), dir, o.reach || 0.3);
      if (s2) pts.push(s2);
    }
    for (let i = 0; i + 1 < pts.length; i++) {
      const w0 = Math.sin(Math.PI * (0.1 + 0.8 * i / (pts.length - 1))), w1 = Math.sin(Math.PI * (0.1 + 0.8 * (i + 1) / (pts.length - 1)));
      C.limb(addv(pts[i], mul(dir, h * w0 - r)), addv(pts[i + 1], mul(dir, h * w1 - r)), r, r, { k, tag: o.tag });
    }
    if (pts.length > 1) C.folds.push({ pts, dirs: pts.map(() => dir), ax: [0, 0, 0], r, deep: o.deep === undefined ? 1 : o.deep, along: true });
    return pts;
  };
