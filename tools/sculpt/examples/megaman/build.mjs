/*
 * Builds the Mega Man model by running megaman.js inside the real app.
 *
 *   node build.mjs            writes megaman.sculpt, megaman.glb and the renders
 *   TARGET=60000 node build.mjs   reduce to a different triangle count first
 *
...
const target = +(process.env.TARGET || 160000);
...
const t0 = Date.now();
await page.addScriptTag({ path: path.join(here, 'megaman.js') });
...
fs.writeFileSync(path.join(here, 'megaman.sculpt'), Buffer.from(files.sculpt, 'base64'));
fs.writeFileSync(path.join(here, 'megaman.glb'), Buffer.from(files.glb, 'base64'));

const shots = { front: [0, 0.05, -0.8, 1.5], 'three-quarter': [0.55, 0.14, -0.8, 1.5],
                back: [Math.PI + 0.5, 0.12, -0.8, 1.5], face: [0.3, 0.06, -0.15, 0.6] };
...
console.log('wrote megaman.sculpt and megaman.glb at', files.tris, 'triangles, plus renders');
await browser.close();
