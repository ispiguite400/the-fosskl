...
(function(){
  const app = M.app, S = window.SCULPT;
  const D = Math.PI / 180;
  // Build a primitive, place it, and weld it into the sculpt with the app's own booleans.
  // prim: 'sphere' | 'roundbox' | 'box' | 'capsule' | 'torus' | {cyl:[radius,height,topScale]}
  M.shape = function (prim, pos, scale, rotDeg, mode, res) {
    let mesh;
    if (typeof prim === 'string') mesh = S.Prim.makeMesh(prim, prim === 'sphere' ? 5 : 12);
    else { const d = S.Prim.cylinder(48, 12, prim.cyl[0], prim.cyl[1], prim.cyl[2]); mesh = new S.Mesh(); mesh.setFromArrays(d.positions, d.indices, { weld: true }); }
    const o = new S.SceneObject('part', mesh);
    o.position.set(pos); o.scale.set(Array.isArray(scale) ? scale : [scale, scale, scale]);
    const r = rotDeg || [0, 0, 0];
    S.Q4.fromEuler(o.rotation, r[0] * D, r[1] * D, r[2] * D);
    o.touch();
    const res2 = S.Boolean.apply(M.obj(), o, { mode: mode || 'union', resolution: res || M.res || 220, smooth: 1 });
    if (!res2.ok) M.log.push('BOOL FAIL ' + prim + ' ' + res2.reason);
    app.afterMeshOp(M.obj());
    return res2;
  };
  // a limb segment between two points
  M.limb = function (prim, a, b, radius, extra, mode) {
    const dx = b[0]-a[0], dy = b[1]-a[1], dz = b[2]-a[2], L = Math.hypot(dx,dy,dz);
    const mid = [(a[0]+b[0])/2, (a[1]+b[1])/2, (a[2]+b[2])/2];
    // rotate +Y onto (dx,dy,dz): about Z then X
    const rz = Math.atan2(-dx, dy) / D, rx = Math.atan2(dz, Math.hypot(dx, dy)) / D;
    if (prim === 'capsule') return M.shape('capsule', mid, [radius / 0.25, L / 1.0, radius / 0.25], [rx, 0, rz], mode);
    return M.shape({ cyl: [radius, L, extra || 1] }, mid, 1, [rx, 0, rz], mode);
  };
})();
(function(){
