...
def main():
    ...
    # tools/build.sh appends PACK_VERSION here; keep whatever it last wrote so a
    # regeneration does not silently reset the build's identity.
    previous = ""
    if os.path.exists(const):
        for line in open(const):
            if "PACK_VERSION" in line:
                previous = line
