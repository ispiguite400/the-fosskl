...
|---|---|---|
| **Understanding** | A natural-language understanding pipeline: collapse multi-word phrases ("fight each other" → one idea), normalise, tokenise, stem, look up a 900-entry lexicon of concepts, correct typos with Damerau-Levenshtein edit distance while refusing to correct names, scope negation, pull out quantities, distances, coordinates, directions, creatures and items, split the message into clauses, then score every possible intent and take the best one — with a confidence, so it can admit it did not follow you. | `scripts/brain/nlu.js` |
| **Acting** | A catalogue of **114 intents**, each mapped onto tasks the body already knows how to perform. | `scripts/brain/orders.js` |
| **Competing** | Contests: races, duels, tournaments, build-offs. Scored off the world, not declared. | `scripts/civ/contest.js` |
| **Living** | The survival ladder they climb when told to please themselves. | `scripts/agent/freeplay.js` |
| **Shaping** | Moats, pits, staircases, walls, roofs, roads — worked out as cell lists, then dug or built. | `scripts/civ/shapes.js` |
...
```
ai! do whatever you want
```

They stop being a miner or a farmer and start being a person dropped into a
world with nothing — and they climb the same ladder you would on your first
evening:

```
chop a tree  →  crafting table  →  wooden pickaxe  →  mine stone
→  stone pickaxe and sword  →  find coal  →  torches
→  dig for iron  →  set up a furnace  →  smelt it  →  iron pickaxe
```

Caught out after dark with no bed, they dig into a bank for the night. Hungry,
they hunt. A rung the world cannot supply — no trees on this island, no sheep
anywhere — is set aside so they get on with something else instead of standing
there. It is a real progression, not a busy-work animation: the test for it
checks their *pockets*, not their captions.

`ai! do as I say` calls them back — all of them, wherever they have got to.

### Challenges

They can compete — against each other, or against the clock. A contest hands
every entrant a real task and then watches the world to see who is winning, so
nobody wins by being told they won.

```
ai! make them fight each other        a melee, last one standing
ai! you two have a duel               a single bout
ai! have a tournament                 a bracket, one pair at a time
ai! first to get 20 wood wins         a gathering race
ai! who can get 10 iron first         same thing, asked as a question
ai! race to that hill                 a foot race to where you are looking
ai! first to kill 5 zombies           a hunt
ai! first one to dig down 20 blocks   a race to depth
ai! build off: who builds a house fastest
ai! who is winning                    the running scores
ai! call it off                       abandon it
```

Fights are **sparring**, not killing: blows are pulled, whoever drops below
half gives best, and everyone is patched up when it ends. A tournament costs
the town nothing.

### Everything you can tell them

**518 commands, every one verified.** `docs/COMMANDS.md` is generated by running
each phrase through the real parser — nothing is listed that does not work.
Type `ai! skills` in game for the short version.
...
**Aim an order at part of the town:**

```
ai! everyone follow me
ai! all the miners dig down 20
ai! three of you go chop wood
ai! half of you go home
ai! Ada and Bram go mine iron
ai! the nearest one come here
```

**Name places and they stay named:**

```
ai! call this place the quarry
ai! go to the quarry          ai! build a road to the quarry
ai! guard the quarry          ai! tunnel to the quarry
```

**Standing orders** fire when the condition comes true, not now:
...
craft, build twelve structures, fight, sleep, form settlements, compete against
each other, take any of the 518 orders above through `/ai:tell`, remember places
you name and words you invent, follow standing orders, and hold conversations
with each other and with you in sentences the pack composes on the spot. Good
game AI, written from scratch. Not a language model.
